...
def test_kicked_watchdog_on_each_core(hpps_serial_per_fnc, host, core_num):
    hpps_serial_per_fnc.sendline("taskset -c " + str(core_num) + " /opt/hpsc-utils/wdtester /dev/watchdog" + str(core_num) + " 1")
...
@pytest.mark.parametrize('core_num', range(8))
def test_unkicked_watchdog_on_each_core(hpps_serial_per_fnc, host, core_num):
    hpps_serial_per_fnc.sendline("taskset -c " + str(core_num) + " /opt/hpsc-utils/wdtester /dev/watchdog" + str(core_num) + " 0")
